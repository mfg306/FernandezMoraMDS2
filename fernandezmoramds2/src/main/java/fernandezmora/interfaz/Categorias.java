...
public class Categorias extends VistaCategorias{
	...
	public void abrirProductosCategoria() {
		...
		/*for(Categoria c : this._list_Categorias) {
			...
			c = new Categoria(_ver_categorias._categorias);
		this._list_Categoria.getCategoria1().addClickListener(event->{
		Productos_categoria pc = new Productos_categoria(c);
		this._ver_categorias._categorias.getLista_categorias().setVisible(false);
		this._ver_categorias._uR_UNR.layout.add(pc);
